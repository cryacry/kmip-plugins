...
func (srv *Server) Serve(l net.Listener) error {
	...
	if !srv.trackListener(ocL, true) {
		return ErrServerClosed
	}
	defer srv.trackListener(ocL, false)
	...
	for {
		rw, e := ocL.Listener.Accept() // 接受连接
		if e != nil {
			if ocL.shuttingDown() || srv.shuttingDown() {
				return ErrServerClosed
			}
			if ne, ok := e.(net.Error); ok && ne.Temporary() { // 如果是网络连接错误 则停顿几秒 继续运行 接收连接
				if tempDelay == 0 {
		...
		tempDelay = 0
		c := &conn{server: srv, rwc: rw} // 封装 rw 连接
...
func (srv *Server) CloseListenersLockedWithAddr(addr string) error {
	srv.mu.Lock()
	defer srv.mu.Unlock()
	var err error
	for ln := range srv.listeners {
		if addr == ipv6toIPv4(ln.Listener.Addr().String()) {
			if cerr := ln.Close(); cerr != nil && err == nil {
				err = cerr
			}
			delete(srv.listeners, ln)
...
func (srv *Server) ListenerList() (list []string) {
	srv.mu.Lock()
	defer srv.mu.Unlock()
	for ln := range srv.listeners {
		addr := ln.Listener.Addr().String()
		list = append(list, ipv6toIPv4(addr))
	}
	return
}
...
func (srv *Server) trackListener(ln *onceCloseListener, add bool) bool {
	srv.mu.Lock()
	defer srv.mu.Unlock()
	if srv.listeners == nil {
		srv.listeners = make(map[*onceCloseListener]struct{})
	}
	if add {
		if srv.shuttingDown() {
			return false
...
func (oc *onceCloseListener) Close() error {
	atomic.StoreInt32(&oc.inShutdown, 1)
	oc.once.Do(oc.close)
	return oc.closeErr
}

func (oc *onceCloseListener) close()             { oc.closeErr = oc.Listener.Close() }
func (oc *onceCloseListener) shuttingDown() bool { return atomic.LoadInt32(&oc.inShutdown) != 0 }
...
func (m *OperationMux) HandleMessage(ctx context.Context, req *Request, resp *Response) {
	ctx = context.WithValue(ctx, TLS, *req.TLS)
